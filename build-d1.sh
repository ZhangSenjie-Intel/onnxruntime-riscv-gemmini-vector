...
export CXX=/home/szhan10/Documents/202107/tools_D1_Nezha_compiler/riscv64-linux-x86_64-20210512/bin/riscv64-unknown-linux-gnu-g++
export CC=/home/szhan10/Documents/202107/tools_D1_Nezha_compiler/riscv64-linux-x86_64-20210512/bin/riscv64-unknown-linux-gnu-gcc
export CXXFLAGS="-march=rv64gcvxtheadc -mabi=lp64d -mtune=c906 -DD1_Nezha"
...
if [ ! -d "build-d1/protoc" ]; then
	mkdir -p "build-d1/protoc"
	curl --location "https://github.com/protocolbuffers/protobuf/releases/download/v3.16.0/protoc-3.16.0-linux-x86_64.zip" --output "build-d1/protoc/protoc.zip"
	cd build-d1/protoc
	unzip protoc.zip
fi
...
python3 $DIR/tools/ci_build/build.py --riscv --skip_submodule_sync --update --build --build_dir=build-d1 "$@"
#ZSJ# python3 -m pdb $DIR/tools/ci_build/build.py --riscv --skip_submodule_sync --update --build --build_dir=build "$@"
